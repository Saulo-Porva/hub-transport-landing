...
### 3. Execute by Wave Schedule

Read the **Wave Schedule** from the DESIGN File Manifest. If no Wave column exists, infer waves from Dependencies.
...
│                                                      │
│  For Wave 1, 2, 3, ... N:                           │
│    ├── Collect all files in this wave               │
│    ├── Delegate all in parallel (if > 1 file)       │
│    ├── Wait for ALL wave files to complete          │
│    ├── Verify each file (ruff, import test)         │
│    └── Only then start next wave                    │
│                                                      │
│  NEVER start Wave N+1 if any Wave N file failed     │
└─────────────────────────────────────────────────────┘
```

**Wave Execution Rules:**
1. Files in the same wave are independent — delegate in parallel
2. A wave is only complete when ALL its files pass verification
3. If any file in a wave fails after 3 retries → stop, mark blocked, document in report
4. Tests (typically Wave 4) always run after all implementation waves

### 3.1 Sealed Test Rule

> **CRITICAL: Before any implementation work, read this rule once.**

```text
SEALED_*.md files exist as holdout acceptance tests.
They were written by define-agent and are evaluated ONLY by ship-agent.

RULE: build-agent MUST NOT read any SEALED_*.md file during phases 1–6.
If you encounter a SEALED_*.md file, skip it — do not open, do not preview.

Rationale: reading sealed scenarios biases implementation toward visible tests,
defeating the shadow score mechanism.
```

### 3.2 Agent Delegation (Framework-Agnostic)
...
After running all verifications, calculate the quality score for the BUILD_REPORT:

```
tests_pass_rate      = tests_passed / total_tests          (from pytest output)
acceptance_pass_rate = AT_passed / AT_total                 (from Acceptance Test Verification table)
no_blockers          = 1.0 if zero blocking issues else 0.0

quality_score = (tests_pass_rate * 0.40) + (acceptance_pass_rate * 0.40) + (no_blockers * 0.20)
```

**Gate:** If `quality_score < 0.90` → do NOT update document statuses, do NOT proceed to `/ship`.
Record the score and blockers in BUILD_REPORT → Quality Score section, then surface to user.
