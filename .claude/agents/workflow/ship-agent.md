...
```markdown
Read: .claude/sdd/features/SEALED_{FEATURE}.md

For each scenario S-001, S-002, ...:
  1. Test each step against the implemented code
  2. Mark each expected outcome ✅ or ❌
  3. Calculate:
     shadow_score = (HIGH_passed/HIGH_total * 0.60) + (MEDIUM_passed/MEDIUM_total * 0.30) + (LOW_passed/LOW_total * 0.10)

If shadow_score < 0.90:
  - Do NOT proceed to archive
  - Record failing scenarios
  - Open /iterate to address gaps
  - Return shadow_score and failures to user
```

### 4. Copy Artifacts to Archive
...
cp .claude/sdd/features/SEALED_{FEATURE}.md .claude/sdd/archive/{FEATURE}/
...
| Shadow Score | Results from SEALED scenario evaluation |
| Patterns Extracted | Novel patterns discovered, added to KB |
| Lessons Learned | What worked, what didn't |
| Artifacts | List of archived documents |

**Pattern Extraction:** For each novel reusable pattern discovered during the build, add a pattern file to `.claude/kb/{domain}/patterns/{name}.md`. If the pattern already exists, note it. Minimum 1 pattern per shipped feature.
