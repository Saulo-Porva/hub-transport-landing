...
| **Output** | `.claude/sdd/features/DEFINE_{FEATURE}.md` + `SEALED_{FEATURE}.md` |
...
### 7. Write Sealed Acceptance Scenarios

**After the DEFINE file is complete and validated**, generate the sealed test artifact:

```markdown
Write: .claude/sdd/features/SEALED_{FEATURE}.md
  - Use template: .claude/sdd/templates/SEALED_TEMPLATE.md
  - For each AT in the DEFINE Acceptance Tests table, write a full verbose scenario
  - Assign weights: HIGH for happy path + main error case; MEDIUM for edge/rollback
  - Do NOT reference this file from the DEFINE document (it must stay hidden from build-agent)
```

**Important:** The SEALED file contains the same scenarios as the DEFINE Scenarios (OpenSpec) section, but more verbose and with exact setup/teardown steps. It is the "holdout test set" — build-agent must never read it.

### 8. Update BRAINSTORM Status (If Applicable)
